...
def gen_images(locs, features, n_gridpoints, normalize=True,
    ...
    n_colors = features.shape[1] / nElectrodes # 3 
...
def datrain(images, labels, fold, model_type, batch_size=32, num_epochs=5):
    ...
    if model_type == '1dconv':
        ...
        [source_class,source_domain,target_domain,target_output_nosm,gd0,gd1] = build_convpool_dalstm(input_var_source,input_var_target,num_classes,100)
    elif model_type == 'lstm':
        network = build_convpool_lstm(input_var, num_classes, 100)
    elif model_type == 'mix':
        network = build_convpool_mix(input_var, num_classes, 100)
...
if __name__ == '__main__':
    ...
    #                                     range(feats.shape[1] / 192)
    #                                     ])
    images_timewin = np.load('../EEGLearn-master/eeglearn/images_timewin.npy')
    #np.save('images_timewin.npz',images_timewin)
    print('\n')
    print('Training the LSTM-CONV Model...')
    #print(len(fold_pairs[2]))
    #print(fold_pairs[2][0])
    #print(fold_pairs[2][1])
    datrain(images_timewin, np.squeeze(feats[:, -1]) - 1, fold_pairs[2], 'dalstm')
