...
for sub_idx = 1:10
    ...
    for target_idx=[1:sub_idx-1 sub_idx+1:10]
        ...
        [X_LDA_, predicted_y_class_all] = lda_apply(X(num_source*2+1:end,:), W, B);
        predicted_y_right = predicted_y_class_all(num_target+1:end);
        predicted_y_left = predicted_y_class_all(1:num_target);
        ...
        acc = sum(predicted_y_left)/num_target/2+sum(predicted_y_right)/num_target/2;
        disp(['acc for initial ', num2str(acc)]);

        [X_LDA, predicted_y_class1] = lda_apply(X(num_source*2+1:num_source*2+num_target,:), W, B);
        
        save target_pred_aft.mat predicted_y_class1
